(function () {
    ...
    (function () {
                ...
                const theme = manualTheme === 'light' || manualTheme === 'dark'
                    ? manualTheme
                    : getSystemThemeEarly();
    ...
    (function () {
        // ==================== 公共配置 ====================
                const FLASK_API_BASE = window.KULEU_FLASK_API_BASE || '';
                // 中文：Flask 版通过同源代理访问酷乐接口，避免浏览器 CORS 与公开代理不稳定影响完整页面功能。
                // English: The Flask version reaches Kuleu through same-origin proxies so browser CORS and unstable public proxies do not break the full page features.
                const JHSJ_API = `${FLASK_API_BASE}/api/kuleu/jhsj`;
                const YINGSHI_API = `${FLASK_API_BASE}/api/kuleu/yingshi`;
                const SHORTDRAMA_RANK_API = `${FLASK_API_BASE}/api/kuleu/shortdramarank`;
                const VTQUARK_API = `${FLASK_API_BASE}/api/kuleu/vtquark?tag=短剧`;
                const ICO_API = `${FLASK_API_BASE}/api/kuleu/ico`;
                const QRCODE_API = `${FLASK_API_BASE}/api/kuleu/qrcode`;
                // 中文：同源代理已经解决跨域问题，因此这里清空公开 CORS 代理，避免重复请求第三方接口导致限流。
                // English: Same-origin proxies already solve cross-origin access, so public CORS proxies stay disabled to avoid duplicated upstream requests and rate limits.
                const CORS_PROXIES = [];
                ...
                const titles = {
                    search: '酷乐短剧 / 影视资源 · <span>聚合搜索</span>',
                    api: '酷乐短剧 / 影视资源 · <span>API生成器</span>',
                    daily: '酷乐短剧 / 影视资源 · <span>每日影视</span>',
                    rank: '酷乐短剧 / 影视资源 · <span>热度榜</span>',
                    favorites: '酷乐短剧 / 影视资源 · <span>我的收藏</span>'
                };
                ...
                function getHashFromPage(pageName) {
                ...
                let suppressNextHashNavigate = false;
                ...
                function switchPageByNav(pageName) {
                    ...
                    /*
                     * 先立即切页面，不等 hashchange。
                     * 这样移动端第一次点击就会马上切换，不需要再点第二次。
                     */
                    navigateTo(pageName);

                    if (window.location.hash !== targetHash) {
                        suppressNextHashNavigate = true;
                        window.location.hash = targetHash;

                        setTimeout(() => {
                            suppressNextHashNavigate = false;
                        }, 0);
                    }
                }
            ...
            `;
                ...
                function showPolicyModalOnEveryPageLoad() {
                    ...
                    /*
                     * 延迟一点打开：
                     * 让路由、页面切换、URL 参数搜索、榜单初始化先完成，
                     * 避免弹窗和首屏渲染抢同一帧，移动端更稳定。
                     */
                    setTimeout(() => {
                        openPolicyModal();
                    }, 600);
                ...
                window.addEventListener('pageshow', event => {
                    if (event.persisted) {
                        ...
                        showPolicyModalOnEveryPageLoad();
                    }
                });
                ...
                async function executeSearchUI(term, options = {}) {
                    ...
                    `;
                        ...
                        autoCollapseRankTagsAfterFirstSuccessfulSearch();
            ...
            `;
                ...
                let hasAutoCollapsedRankAfterSearch = false;
                ...
                function autoCollapseRankTagsAfterFirstSuccessfulSearch() {
                    if (hasAutoCollapsedRankAfterSearch) return;

                    hasAutoCollapsedRankAfterSearch = true;
                ...
                function updateThemeToggleButton(theme) {
                    ...
                    // 中文：按钮只显示浅色/深色图标，不再写“开灯/关灯”，避免主题含义被误读。
                    // English: The switch exposes light/dark icons only, avoiding misleading on/off wording in the UI.
                    themeToggleBtn.dataset.currentTheme = isLight ? 'light' : 'dark';
                    themeToggleBtn.title = isLight ? '当前为浅色图标，点击切换为深色图标' : '当前为深色图标，点击切换为浅色图标';
                    themeToggleBtn.setAttribute('aria-label', isLight ? '当前为浅色图标，点击切换为深色图标' : '当前为深色图标，点击切换为浅色图标');
                    themeToggleBtn.setAttribute('aria-pressed', isLight ? 'true' : 'false');
                }

                function applyTheme(theme, options = {}) {
                    const { save = false, showToast = false } = options;
                ...
                function switchThemeWithAnimation(nextTheme, event) {
                    ...
                    /*
                     * 支持 View Transition 的浏览器：
                     * 浏览器会自动截取旧页面和新页面快照。
                     * 真实 DOM 瞬间完成主题切换，用户看到的是快照整体淡入淡出。
                     */
                    if (document.startViewTransition) {
                        ...
                        transition.finished.finally(() => {
                            setTimeout(unlockThemeSwitch, 40);
                        });
                    ...
                    window.setTimeout(unlockThemeSwitch, THEME_SWITCH_UNLOCK_DELAY);
                }
