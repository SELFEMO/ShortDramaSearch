// 中文：Console 版本复用稳定的功能逻辑，但由独立 CSS 接管视觉设计，避免重写搜索、收藏、API 与弹窗流程引入回归风险。
// English: The Console version reuses the stable functional logic while a standalone CSS layer owns the redesign, avoiding regressions from rewriting search, favorites, API, and modal flows.
(function () {
    ...
    (function () {
        // ==================== 公共配置 ====================
            ...
            const titles = {
                search: '酷乐短剧 / 影视资源 Console · <span>聚合搜索</span>',
                api: '酷乐短剧 / 影视资源 Console · <span>API生成器</span>',
                daily: '酷乐短剧 / 影视资源 Console · <span>每日影视</span>',
                rank: '酷乐短剧 / 影视资源 Console · <span>热度榜</span>',
                favorites: '酷乐短剧 / 影视资源 Console · <span>我的收藏</span>'
            };
            ...
            function navigateTo(pageName, options = {}) {
                ...
                if (isSamePage && options.force !== true) {
                    ...
                    updatePageScrollbar();
                    if (typeof window.neoRequestCondensedNavConsole === 'function') {
                        ...
                        window.requestAnimationFrame(window.neoRequestCondensedNavConsole);
                    }
                    return;
                }
            ...
            function switchPageByNav(pageName) {
                ...
                window.requestAnimationFrame(() => {
                    ...
                    if (typeof window.neoRequestCondensedNavConsole === 'function') {
                        ...
                        window.neoRequestCondensedNavConsole();
                    }
                });
            }
            ...
            function renderSearchHistory() {
                ...
                searchHistoryList.innerHTML = list.map(item => {
                    // 中文：历史项会直接触发再次搜索，因此保留 button 语义，同时增加标题与可访问标签让重设计后的命令胶囊仍然清晰可用。
                    // English: History items trigger a repeat search directly, so button semantics are preserved while title and accessible labels keep the redesigned command capsule clear and usable.
                    return `<button type="button" class="history-chip" title="再次搜索：${escapeHtml(item)}" aria-label="再次搜索：${escapeHtml(item)}" data-history-keyword="${escapeHtml(item)}">${escapeHtml(item)}</button>`;
                }).join('');
            }

            function setConsoleStat(el, kicker, count, label, detail = '') {
                if (!el) return;

                // 中文：统计信息属于页面状态而不是普通段落，统一用结构化胶囊输出，避免在 Console 页面中漂浮成未设计文本。
                // English: Stats are page state rather than plain paragraphs, so they are rendered as structured capsules to avoid floating undesigned text in the Console UI.
                el.innerHTML = `
                    <span class="console-stat-kicker">${escapeHtml(kicker)}</span>
                    <strong>${escapeHtml(String(count))}</strong>
                    <span>${escapeHtml(label)}</span>
                    ${detail ? `<span class="console-stat-muted">${escapeHtml(detail)}</span>` : ''}
                `;
            }
            ...
            function renderFavorites() {
                ...
                setConsoleStat(favoritesStats, 'FAV', list.length, '条收藏', keyword ? `筛选 ${filtered.length} 条` : '本地保存');

                if (!filtered.length) {
                    const emptyTitle = keyword ? '没有匹配的收藏' : '暂无收藏内容';
                    const emptyDescription = keyword
                        ? `没有找到包含“${keyword}”的本地收藏，请换一个关键词筛选。`
                        : '收藏仅保存在当前浏览器本地。你可以先回到聚合搜索页，在资源卡片中点击“收藏”。';

                    // 中文：空收藏是一个页面状态，需要明确标题、说明和下一步动作，而不是贴边显示一行普通提示。
                    // English: Empty Favorites is a page state that needs a clear title, explanation, and next action instead of a single cramped plain-text hint.
                    favoritesList.innerHTML = `
                        <div class="favorite-empty console-empty-card">
                            <span class="console-empty-kicker">FAV EMPTY</span>
                            <strong>${escapeHtml(emptyTitle)}</strong>
                            <p>${escapeHtml(emptyDescription)}</p>
                            <a class="console-empty-action" href="#/">去聚合搜索</a>
                        </div>
                    `;
                    if (typeof window.neoRequestCondensedNavConsole === 'function') {
            ...
            async function executeSearchUI(term, options = {}) {
                ...
                try {
                    ...
                    if (typeList.length === 0 || total === 0) {
                        setResultsAreaMode('is-feedback');
                        // 中文：空结果也使用控制台反馈卡片，避免孤立文字看起来像页面布局缺失。
                        // English: Empty results use the console feedback card as well, avoiding a lone text line that looks like missing layout.
                        resultsArea.innerHTML = `
                            <div class="status-message status-message-empty">
                                <span class="status-message-kicker">NO MATCH</span>
                                <strong>未找到相关结果</strong>
                                <p>可以换一个关键词，或把来源切回“全局聚合”再次检索。</p>
                            </div>
                        `;
                        showStatus('未找到相关结果');
                        return;
                    ...
                    // 中文：Console 设计版把热度榜视为持续可用的搜索入口，搜索成功后不自动收起，避免用户需要反复手动展开。
                    // English: Console treats the ranking list as a persistent search entry, so it is not auto-collapsed after a successful search to avoid repeated manual expansion.
            ...
            function renderGroupedResults(mergedByType, typeList, term, total, apiStatus = null) {
                ...
                // 中文：Console 版把结果摘要、来源状态与失效过滤合并为一个控制面板，避免信息散落成普通文本后缺乏层级。
                // English: The Console version groups result summary, source health, and broken-link filtering into one control panel so the information does not appear as flat text.
                let html = `
                <div class="result-command-panel" aria-label="搜索结果概览">
                    <div class="result-command-main">
                        <span class="result-command-eyebrow">SEARCH OUTPUT</span>
                        <strong>关键词：${escapeHtml(term)}</strong>
                        <p>已匹配 ${total} 条结果，命中来源：${escapeHtml(typeNames || '无')}</p>
                    </div>
                    <div class="result-command-meta" aria-label="接口状态">
                        <span class="result-count-chip">共 ${total} 条</span>
                        ${apiStatus ? `<span class="search-health-chip ${apiStatus.failedCount ? 'warn' : 'ok'}">
                            请求 ${apiStatus.successCount}/${apiStatus.successCount + apiStatus.failedCount}
                        </span>` : ''}
                    </div>
             ...
             * 短剧热度榜状态：
             * - 首次打开网页：默认展开前 10 条
             * - Console 设计版搜索成功后保持当前展开状态，不再沿用旧版自动折叠机制
             */
            ...
            function renderRankList() {
                ...
                if (filtered.length === 0) {
                    rankListContainer.innerHTML = `<div class="status-message-empty"><strong>NO MATCH</strong><p>没有找到相关内容。</p></div>`;
                    return;
                }
                ...
                filtered.forEach((item, index) => {
                    ...
                    const rankColor = Number(rank) <= 3 ? 'var(--accent)' : 'var(--muted)';

                    // 中文：热榜卡片是“可执行搜索命令”，因此把按钮上移到标题同层，减少大面积空白并强化下一步操作。
                    // English: Ranking cards are executable search commands, so the action is lifted beside the title to reduce empty space and emphasize the next step.
                    html += `<div class="result-item rank-result-item"
                            data-keyword="${escapeHtml(title)}"
                            style="animation-delay: ${index * 0.03}s">

                            <div class="drama-title" style="cursor: pointer;">
                                <span class="tag-rank" style="margin-right: 5px; font-size: 14px; color: ${rankColor};">
                            ...
                            <div class="rank-result-meta">
                                ...
                                <span class="rank-heat-value">🔥 ${escapeHtml(hot)}</span>
                            </div>

                            <div class="rank-result-action">
            ...
            async function loadDailyResources() {
                setConsoleStat(dailyStats, 'DAILY', '…', '加载中');
                dailyResultsArea.innerHTML = `<div class="status-message-loading"><div class="spinner"></div><strong>LOADING</strong><p>正在加载今日影视资源...</p></div>`;
                try {
            ...
            function renderDailyResults(data) {
                if (!data || data.length === 0) {
                    setConsoleStat(dailyStats, 'DAILY', 0, '影视资源', '暂无数据');
                    dailyResultsArea.innerHTML = `<div class="status-message-empty"><strong>NO DATA</strong><p>未获取到今日影视资源。</p></div>`;
                    return;
                }
                ...
                setConsoleStat(dailyStats, 'DAILY', data.length, '条影视资源', sortedSources.length ? `来源 ${sortedSources.length}` : '');
                let html = '';
            ...
            function updateThemeToggleButton(theme) {
                ...
                // 中文：按钮可见区域显示浅/深端点，同时在无障碍标签里说明当前是否跟随系统。
                // English: The visible switch shows Light/Dark endpoints while accessible labels still expose whether the theme follows system or manual mode.
            ...
            function bindSystemThemeAutoConsole() {
            ...
            function bindNeoCondensedNav() {
                ...
                function requestCondensedConsole() {
                    if (navRaf) return;
                ...
                window.neoRequestCondensedNavConsole = requestCondensedConsole;

                if (window.MutationObserver && document.body) {
                    const pageObserver = new MutationObserver(requestCondensedConsole);
                ...
                if (window.ResizeObserver && mainContent) {
                    const contentObserver = new ResizeObserver(requestCondensedConsole);
                ...
                window.addEventListener('scroll', requestCondensedConsole, { passive: true });
                window.addEventListener('resize', requestCondensedConsole);
                window.addEventListener('orientationchange', () => window.setTimeout(requestCondensedConsole, 120));
                syncCondensedState();
            }
